...
| **Arc state** | Main `7f20e44`. TS.tsx 15,871 → 10,928 (−4,943, −31.1%). Test suite: 550 passing / 6 pre-existing fails unchanged. Invoice cluster CLOSED. IIF DELETED. InvoiceDetailModal EXTRACTED. Convera batch CSV builder EXTRACTED (X6 partial). | 🟢 in progress | S6 | Next: **CB** (Convera Batch modal extract, half-gated per X6 partial). Then IB (Intuit Batch modal). Then 🛑 STOP GATE (QB Automation talk with Dan) before X4/QA7/QA8/QA9. QE/PI deps on X7 IIF need re-scope. |
