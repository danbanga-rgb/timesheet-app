...
| **✅ STOP GATE RESOLVED** | Talk happened 2026-09-18. Outcome: **pivot to v2 rebuild.** Chunk 8 (X4 + QA1–QA9) SUPERSEDED. v1 QB Auto stays frozen; v2 built ground-up at `src/roles/Accountant/tabs/QbAutomationV2/`, admin-gated, in parallel with prod v1. Reuse plumbing (already-extracted modals + wrapper handlers + 4-layer lib); rebuild UI with QbExport-card design language. Umbrella mapping fixed in v2. Modularization arc PAUSED while v2 is spec'd + built; Chunks 9/10 and Phase 6 remain valid when arc resumes. See [[qb-automation-v2-pivot]] + `.claude/plans/qb-automation-v2.md`. | ✅ PIVOT | S7 (2026-09-18) | **Next: Dan gives v2 directionality → Claude asks questions → design doc written.** |
| **Arc state (pre-pivot snapshot)** | Main `801b080`. TS.tsx 15,871 → 10,431 (−5,440, −34.3%). Test suite: 550 passing / 6 pre-existing fails unchanged. All slices through IB shipped. Invoice cluster CLOSED. IIF DELETED. InvoiceDetailModal EXTRACTED. Convera + Intuit batch flows EXTRACTED. | ✅ | S6 EOD | Snapshot at pivot moment. Arc paused; v2 build begins next. |
...
### Chunk 8 — QB Automation tab (TS.tsx 9843–11560, ~1,717 lines) — ⚠️ SUPERSEDED 2026-09-18

> **✅ STOP GATE RESOLVED (2026-09-18) → PIVOT.** Talk happened; outcome: **do NOT extract v1.** Build ground-up v2 at `src/roles/Accountant/tabs/QbAutomationV2/`, admin-gated, in parallel with prod v1. Chunk 8 slices X4 + QA1–QA9 are SUPERSEDED. Kept below for archaeology / reuse when v2 needs to inventory v1 handler surface. See `.claude/plans/qb-automation-v2.md` for the v2 arc + [[qb-automation-v2-pivot]] memory.
