...
| **Arc state** | Main `89f6dfa`. TS.tsx 15,871 → 10,539 (−5,332, −33.6%). Test suite: 550 passing / 6 pre-existing fails unchanged. Invoice cluster CLOSED. IIF DELETED. InvoiceDetailModal EXTRACTED. Convera batch CSV builder EXTRACTED. ConveraBatchModal EXTRACTED. | 🟢 in progress | S6 | Next: **IB** (Intuit Batch modal extract, ~2h Medium risk). Then 🛑 STOP GATE (QB Automation talk with Dan) before X4/QA7/QA8/QA9. QE/PI deps on X7 IIF need re-scope. |
