...
serve(async (req) => {
  ...
  // Reject internal forwarder emails — should never be used as contractor identity
  const internalDomain = Deno.env.get('INTERNAL_DOMAIN') || '';
  if (internalDomain && (contractorEmail as string).toLowerCase().endsWith('@' + internalDomain)) {
    return new Response(JSON.stringify({ error: `Rejected: contractorEmail is an internal address (${contractorEmail})` }), {
      status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
